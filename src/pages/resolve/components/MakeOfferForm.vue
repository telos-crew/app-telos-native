	...
	<q-card class="make-offer-form">
		...
		<q-card-section class="q-pt-none">
			<q-input
				...
				bottom-slots
				autofocus
				:rules="[(val) => val > 0 || $t('pages.resolve.make_offer_hours_hint')]"
			/>
		</q-card-section>
		<q-card-section class="q-pt-none">
			<q-input
				...
				bottom-slots
				:rules="[(val) => val > 0 || $t('pages.resolve.make_offer_rate_hint')]"
			/>
		</q-card-section>

		<p class="total">
			{{ $t('pages.resolve.make_offer_estimate_total', { total }) }}
		</p>
		...
		>
			<q-btn
				flat
				:label="$t('pages.resolve.make_offer_submit')"
				@click="submit"
			/>
			<q-btn
				flat
				:label="$t('pages.resolve.make_offer_cancel')"
				@click="close"
			/>
		</q-card-actions>
	</q-card>
