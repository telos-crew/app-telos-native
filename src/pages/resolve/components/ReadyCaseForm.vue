<template>
	<q-card class="ready-case-form">
		...
		<q-card-section class="q-pt-none">
			...
			<p>
				<strong>
					{{
						$t('pages.resolve.ready_case_deduction', {
							tlosFee: tlosFee.toFixed(4),
							balance
						})
					}}
				</strong>
			...
			>
				<strong>
					{{
						$t('pages.resolve.ready_case_deficit', {
							balance,
							adjustedDeficit: this.adjustedDeficit
						})
					}}
				</strong>
			</p>
    ...
    {{ exchangeRate }} tlosFee: {{ tlosFee }} deficit: {{ deficit }} -->
		...
		>
			<q-btn
				flat
				:label="$t('pages.resolve.ready_case_ready')"
				@click="submit"
			/>
			<q-btn
				flat
				:label="$t('pages.resolve.ready_case_cancel')"
				@click="close"
			/>
		</q-card-actions>
	</q-card>
