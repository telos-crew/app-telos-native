	...
	<div class="case-file-action">
		...
		<div v-if="act.name === 'acceptclaim'">
			<i18n-t keypath="pages.resolve.case_action_accept">
				...
				<template #class
					><strong>{{
						decisionClass(actionData.decision_class)
					}}</strong></template
				>
			</i18n-t>
			({{ action.timestamp }})
		...
		<div v-if="act.name === 'makeoffer'">
			<i18n-t keypath="pages.resolve.case_action_make_offer">
				...
				<template #hours
					...
					><strong>{{ actionData.estimated_hours }}</strong></template
				>
				<template #rate
		...
		<div v-if="act.name === 'setruling'">
			<i18n-t keypath="pages.resolve.case_action_set_ruling">
				...
				>
				<template #link
					><a :href="createIpfsLink(actionData.case_ruling)">ruled</a></template
				>
			</i18n-t>
			<strong>{{ authorization[0].actor }}</strong> (admin)
			<a :href="createIpfsLink(actionData.case_ruling)">{{
				$t('pages.resolve.ruled')
			}}</a>
			on case ({{ action.timestamp }})
		</div>
