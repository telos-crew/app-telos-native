	...
	data() {
		...
		return {
			rules: {
				...
				accountLength: (val) =>
					off || val.length <= 12 || this.$t('forms.errors.accountLength'),
				dateFuture: (date) => (val) =>
					off ||
				...
				positiveInteger: (val) =>
					off || parseInt(val) > 0 || this.$t('forms.errors.positiveInteger'),
				isValidIPFShash: (val) =>
					off || val.match(regex) || 'Invalid IPFS HASH format',
