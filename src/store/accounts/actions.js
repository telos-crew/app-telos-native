...
const getAuthenticator = function (ual, wallet = null) {
	...
	const idx = ual.authenticators.findIndex((auth) => auth.getName() === wallet)
	return {
...
export const memoryAutoLogin = async function ({ dispatch, rootState }) {
...
export const verifyOTP = async function ({ state }, { password, publicKey }) {
	try {
...
export const getAccount = async function ({ rootState, commit }) {
	...
	if (rootState.accounts.account) {
		...
		accountData.core_liquid_balance =
			accountData.core_liquid_balance || '0.0000 TLOS'
	}
